...
const fshader2 = `
  ...
  vec4 twirl(sampler2D tex, vec2 uv, float time){
    ...
    // 半径以下の場合の条件分岐
    if (dist < 0.5){
      // 半径と現在地の差分（距離がcenterから遠いほど小さくなる）
      float delta = (0.5 - dist);
      // delta * time * u_twirlsをthetaとし回転をきめる
      float theta =  delta * time * u_twirls;
      float s = sin(theta);
      float c = cos(theta);
    ...
    // += centerとすることで少しずつセンターから離れた色を取得してく
    tc += center;

    vec4 color = texture2D(tex, tc);

    // rgbaのaにmix関数で下記の値を入れるとフェードインする
...
// textをmeshにしてくれるlidsのtoon3ファイルで定義してある
